...
function safeAuthOpenFlags(constants) {
  if (!constants
...
async function copyAuthIfPresent({ sourceHome, codexHome, flags }) {
  let auth;
  try {
    auth = await fs.open(path.join(sourceHome, 'auth.json'), flags);
    const stat = await auth.stat();
    if (!stat.isFile() || stat.size > MAX_AUTH_BYTES || (stat.mode & 0o077) !== 0) {
      throw new Error('Codex authentication file is invalid');
    }
    const contents = await readBoundedAuth(auth);
    await fs.writeFile(path.join(codexHome, 'auth.json'), contents, { mode: 0o600, flag: 'wx' });
  } catch (error) {
    if (error?.code === 'ENOENT') return;
    if (error?.message === 'Codex authentication file is invalid') throw error;
    throw new Error('Codex authentication file is invalid', { cause: error });
  } finally {
    await auth?.close();
  }
}
...
export async function prepareCodexLaunchEnvironment({
  ...
  const authFlags = safeAuthOpenFlags(fsConstantsImpl);
  const codexHome = path.join(workspaceDir, 'codex-home');
  let created = false;
  try {
    await fs.mkdir(codexHome, { mode: 0o700 });
    created = true;
    await fs.writeFile(path.join(codexHome, 'config.toml'), ISOLATED_CONFIG, { mode: 0o600, flag: 'wx' });

    const userHome = env.HOME || env.USERPROFILE || os.homedir();
    const sourceHome = env.CODEX_HOME || path.join(userHome, '.codex');
    await copyAuthIfPresent({ sourceHome, codexHome, flags: authFlags });
  } catch (error) {
    if (created) {
      try {
