...
Use this skill for secure SDLC work, design review before implementation, vulnerability validation in application code, and practical remediation guidance for developers.

## Use another skill when

Choose another skill first for active incident handling, cloud landing-zone design, or offensive exploit simulation.

## Operating guardrails

- Authorized security testing and secure code review only.
- Defensive application security, developer enablement, and secure SDLC contexts are in scope.
...
- authorized scope and target system
- the main risk, review question, or decision to make
...
const requireAuth = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const jwtVerificationKey = await keyProvider.get('jwt-public-key'); // Secrets manager or KMS-backed config
    req.user = jwt.verify(token, jwtVerificationKey) as UserClaims;
    next();
  } catch {
